...
`.gitignore` は既に追跡されたファイルを解除しません。上記の確認で見つかった場合は、秘密値を出力せず対象を確認し、必要なら `git rm --cached -- <対象ファイル>` で追跡だけを解除します。履歴に実際の秘密情報が入っていた場合は、削除だけでなく失効・再発行と影響調査が必要です。今回の作業開始時には追跡済みファイルはありませんでした。
...
GitHub 上での実行は今回の作業範囲外で、次の確認が必要です。

- push と PR の両方で `Secret scan` / `Gitleaks` が起動・成功し、ログのバージョン、Linux 配布物のチェックサム照合、検査対象を確認する。fork PR の初回実行など、承認待ちがある場合は実行状態を区別する。
