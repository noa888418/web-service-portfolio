...
AWS の月額予算 **3,000 円**、公開は **月 60 時間程度・事前に案内した期間のみ**、独自ドメイン未所有は決定・確認済みです。見積もりでは準備・検証・撤去も合計 60 時間に含める案とし、公開そのものを 60 時間確保する場合は追加費用を確認します。

東京、CloudFront 従量課金・標準ドメイン HTTPS、VPC origin → 非公開 ALB、public IPv4 で外向き通信する Fargate、非公開 Single-AZ RDS、NAT Gateway・Redis なしは [基本設計案](docs/architecture.md)として整理しました。内部 HTTP の許容、データ保存・復元、運用手順はレビューが必要です。
...
| 技術の詳細 | 各製品の対応バージョン、テストツール、API・認証の詳細設計 |
| AWS の詳細 | 東京・CloudFront・Fargate・RDS 等の案の承認、AZ・サイズ・内部暗号化範囲、公開デモの利用者制限、データ保存・復元・削除、通知先・撤去実行者 |
...
2. **AWS 基本設計の許容範囲**：内部 HTTP、標準証明書の制約、単一タスク・Single-AZ、公開デモのアクセス制限を確認する。
3. **時間・データ運用**：準備を含む 60 時間の配分、デモ投入許可、保存・復元・削除方針、予算通知先と撤去実行者を決める。
